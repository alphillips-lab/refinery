#!/usr/bin/env python3
# -*- coding: utf-8 -*-
from Cryptodome.Cipher import ARC4
from refinery.units import Unit


class msdqt(Unit):
    """
    Decrypt Microsoft Defender Quarantined files
    """
    KEY = bytes([
        
    ])

    def process(self, data: bytes) -> bytes:
        if data[:3] != b"\x0B\xAD\x00":
            msdqt.log_warn("Invalid or unknown MS Defender Quarantined file header magic")
            return b""
        rc4 = ARC4.new(self.KEY)
        output = rc4.decrypt(data)
        orig_length = 0x28 + int.from_bytes(output[8:12], "little")

        return output[orig_length:]
